...
def distances(positions):
    ...
    for i in range(dim):
        for j in range(dim):
            ...
            dist[i, j] = np.linalg.norm(diff) + np.random.uniform(-5, 5)
    return dist
...
# TEST
#startFormation = pd.read_csv("startFormation.csv", delimiter=";", header=None)
#currentFormation = pd.read_csv("nextStep.csv", delimiter=";", header=None)
#startFormation_matrix = startFormation.to_numpy()
#currentFormation_matrix = currentFormation.to_numpy()
#
#index = 0
#movement = currentFormation_matrix[index] - startFormation_matrix[index]
#currDistances = distances(currentFormation_matrix)
#
#new_positions = calculate_relative_positions(startFormation_matrix, currDistances, index, movement)
#offset = currentFormation_matrix[index].copy()
#for i in range(currentFormation_matrix.shape[0]):
#    currentFormation_matrix[i] = currentFormation_matrix[i] - offset
#print(currentFormation_matrix)
#print(new_positions)